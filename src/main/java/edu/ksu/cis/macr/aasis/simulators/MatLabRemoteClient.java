...
import edu.ksu.cis.macr.aasis.simulators.model.SimulatorData;
...
public class MatLabRemoteClient {
     ...
     */
    public List<SimulatorData> receiveData(int count) throws IOException, ClassNotFoundException {
        ...
        List<SimulatorData> data = new ArrayList<>();
        InputStream is = connection.getInputStream();
        ObjectInputStream ois = new ObjectInputStream(is);

        SimulatorData ds = null;
        for (int i = 0; i < count; i++) {
            ds = (SimulatorData) ois.readObject();
     ...
     */
    public void sendData(File file, String meterName) throws IOException {
        connection = new Socket(address, portNo);
        out = new PrintWriter(connection.getOutputStream());

 ...
 */
public class MatLabSimulator implements ISimulator {
    private static final Logger LOG = LoggerFactory.getLogger(MatLabSimulator.class);
    ...
    public MatLabSimulator() {
        ...
        if (MatLabSimulator.matLabConnection == null) {
            if (useRemoteConnection) {
                ...
                try {
                    MatLabSimulator.matLabConnection = new MatLabConnection();
    ...
    public static void updateAllSensors(long timeSlice) throws Exception {
        ...
        if (timeSlice == 0) {
            ...
            MatLabSimulator.updateMatricesForNewTimeSlice(timeSlice);
        }
        ...
        if (debug)
            LOG.debug("MatLab: writing system generation data to {}. WriteFiles is set to {}.", outFile1, writeFiles);
        ...
        if (debug)
            LOG.debug("MatLab: writing Net.Sub array data to {}. WriteFiles is set to {}.", outFile2, writeFiles);
        ...
        MatLabSimulator.setSensor_Data_NewInputs(Sensor_Data_partial);
        ...
        if (timeSlice == 0) {
            ...
            try {
                for (double[] doubles : MatLabSimulator
                        .Sensor_Data_Denise = Sensor_Data_NewInputs.clone()) {
                }
                ...
                for (double[] doubles : MatLabSimulator.allSensorsNow =
    ...
    private static void updateMatricesForNewTimeSlice(long timeSlice) {
        ...
        if (debug)
            LOG.debug("MatLab at t {}: Before executing, the full data set is {}", timeSlice, allSensorsPrevious);
        ...
        MatLabSimulator.Sensor_Data_Denise = Sensor_Data_NewInputs;
        if (debug)
    ...
    public static void setIsConnected(final boolean isConnected) {
        MatLabSimulator.isConnected = isConnected;
    }

    /*
     * Start a remote connection with the Server on the MatLab machine.
     */
    public void remoteConnect(String defaultScenario) {
    ...
    public String toString() {
        return "MatLabSimulator (running MatLab instance)";
    }
    ...
    public ArrayList<IRead<?>> getAllReadsAt(long timeSlice) {
        ...
        try {
            MatLabSimulator.updateAllSensors(timeSlice);
...
//    for (int i = 0; i < MatLabSimulator.ieee37_temp.length; i++) {
//      IRead smartMeterRead = getReadByRow(i);
...
//    }
     ...
     */
    @Override
    public IRead<?> getRead(String sensorName) {
    ...
    public IRead<?> getReadAt(String sensorName, long timeSlice) {
        ...
        IRead<?> read = new Read<SensorType>();
        if (timeSlice >= 0 && !sensorName.isEmpty()) {
            ...
            int rowID = RowTranslator.getRowID(sensorName);
            if (rowID == -1) {
            ...
            IRead dataRow = getReadByRow(rowID);
            read.setSensorReadObject(dataRow);
    ...
    * .lang.String)
     */
    @Override
    public void initialize(String defaultScenario,
        ...
        if (isUseRemoteConnection()) {
            ...
            if (debug)
                LOG.debug("Reading simulated data for {} meters from running MatLab instance. ", meterName.size());
        }
        isInitialized = true;
    }
    ...
    public boolean issueControlAction(final String meterName, final long timeSlice, final ISetting<?> setting) {
        ...
        final int iRow = RowTranslator.getRowID(meterName);

        // get the column number for just the calculated generation reactive
        // power, Q
        final int iCol = ColumnTranslator.getColID("Qgeneration");

        // update the data
        double[][] arr = MatLabSimulator.getSensor_Data_NewInputs().clone();
        if (debug) LOG.debug("CELL BEFORE: {}", arr[iRow][iCol]);
        // arr[iRow][iCol] = ((Setting) setting.getActuatorSettingObject()).getQ_gen_now();
        arr[iRow][iCol] = iRow + iCol; // dummy data
        if (debug) LOG.debug("CELL AFTER: {}", arr[iRow][iCol]);
        MatLabSimulator.setSensor_Data_NewInputs(arr);
        MatLabSimulator.setSensor_Data_Denise(arr);
    ...
    */
    ...
    private IRead getReadByRow(int iRow) {
        if (debug) LOG.debug("Looking up the data for row {}.", iRow);
